...
import { Eye, EyeOff, Mail, Lock, GraduationCap, Users, ArrowLeft } from 'lucide-react';
...
export function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { success, message: loginMessage, user: loggedInUser } = await login(email, password, role || undefined);
    ...
    if (success && loggedInUser) {
      ...
      setError(loginMessage || 'Invalid email or password');
    }
  };
      ...
      <Card>
        <CardHeader className="space-y-1">
          ...
          <CardDescription className="text-center">
            Sign in to your LearnFlux account
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <Button
            type="button"
            variant="outline"
            className="w-full flex items-center justify-center gap-3 py-5 border-2 hover:bg-accent/50 transition-all"
            onClick={() => {
              window.open(
